 ...
 * MySQLPump<br>
 * 
 * MySQLPump: Implements MySQL backups with mysqlpump command<br><br>
 * 
 ...
 */
public class MySQLPump extends Backup {
	/**
	 ...
	 */
	public MySQLPump(Properties config) {
		this._config = config;
	}
	...
	/**
	 * Execute backup with mysqlpump<br><br>
	 * 
	 * Notes: mysqlpump must be accessible<br><br>
	 * 
	...
	public boolean backup(String fileName) {
		...
		StringBuffer command = new StringBuffer("mysqlpump");
		...
		String dp = this._config.getProperty("PARALLELISM", "4");
		...
		command.append(" -P ");
		command.append(port);
		command.append(" --default-parallelism=");
		command.append(dp);
		command.append(" ");
